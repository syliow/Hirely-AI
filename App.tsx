        ...
        {loading || refactoring ? (
          ...
          <div className="space-y-16 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              <section className="lg:col-span-2 bg-white/90 dark:bg-[#080d1a]/90 backdrop-blur-2xl border border-slate-200 dark:border-white/10 rounded-[56px] p-12 lg:p-20 shadow-2xl">
                <div className="flex flex-col items-center text-center space-y-14 max-w-5xl mx-auto">
                  <div className="space-y-4">
                    <div className="flex items-baseline justify-center gap-4">
                      <span className="text-[10rem] font-black text-slate-900 dark:text-white tracking-tighter leading-none">{result.overall_score}</span>
                      <span className="text-4xl font-bold text-slate-400 dark:text-slate-600">/ 100</span>
                    </div>
                    <h3 className="text-sm font-black uppercase tracking-[0.5em] text-violet-500">Professional Readiness Index</h3>
                  </div>

                  <div className="space-y-6 w-full">
                    <h4 className="text-sm font-black uppercase tracking-widest text-slate-400 dark:text-slate-600">Executive Summary</h4>
                    <p className="text-xl md:text-2xl font-medium leading-relaxed text-slate-700 dark:text-slate-200 px-8">"{result.summary}"</p>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-20 gap-y-12 w-full pt-10">
                  ...
                  <div className="w-full pt-14 flex flex-col items-center gap-8 relative">
                    <div className="relative w-full max-w-xl">
                      {readyHtml ? (
                         <button onClick={() => handleOpenPreview(readyHtml)} className="w-full flex items-center justify-center gap-6 px-12 py-8 bg-emerald-500 hover:bg-emerald-400 text-white rounded-[32px] text-base font-black uppercase tracking-[0.25em] shadow-2xl shadow-emerald-500/40 transition-all active:scale-95 animate-bounce">
                           <Eye className="w-8 h-8" /> View Optimized Resume
                         </button>
                      ) : (
                        <div className="relative">
                          <button onClick={() => setShowRefactorOptions(!showRefactorOptions)} className="w-full flex items-center justify-center gap-6 px-12 py-8 bg-violet-600 hover:bg-violet-500 text-white rounded-[32px] text-base font-black uppercase tracking-[0.25em] shadow-2xl shadow-violet-500/40 transition-all active:scale-95">
                            <Download className="w-8 h-8" /> Download Resume
                          </button>
                          {showRefactorOptions && (
                            <div className="absolute bottom-full left-0 right-0 mb-8 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/10 rounded-[32px] p-4 shadow-3xl z-[100] animate-in fade-in slide-in-from-bottom-8">
                              <button onClick={() => handleRefactor('pdf')} className="w-full flex items-center gap-5 px-8 py-6 hover:bg-slate-50 dark:hover:bg-white/10 rounded-2xl text-sm font-black uppercase tracking-widest text-slate-700 dark:text-slate-200 transition-colors">
                                <Sparkles className="w-6 h-6 text-violet-500" /> Optimize into PDF
                              </button>
                              <button onClick={() => handleRefactor('docx')} className="w-full flex items-center gap-5 px-8 py-6 hover:bg-slate-50 dark:hover:bg-white/10 rounded-2xl text-sm font-black uppercase tracking-widest text-slate-700 dark:text-slate-200 transition-colors">
                                <FileDown className="w-6 h-6 text-blue-500" /> Download as Word (.docx)
                              </button>
                            </div>
                          )}
                        </div>
